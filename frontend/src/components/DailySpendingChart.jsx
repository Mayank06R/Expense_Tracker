...
export default function DailySpendingChart() {
  ...
  const avg = (() => {
    ...
    return expenses.length ? expenses.reduce((a, b) => a + b, 0) / expenses.length : 0;
  })();
  const daysLogged = series.slice(0, today).filter((s) => s.expense > 0).length;
  ...
  }, [series.length, monthRef.getMonth()]);
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: "#7A7A83" }}>
            {monthRef.toLocaleDateString("en-IN", { month: "long", year: "numeric" })} — amount per day
          </p>
        </div>
        <span className="chip">
      ...
      <div className="mt-6 flex gap-3" style={{ height: 220 }}>
        {/* Y axis */}
        <div className="flex flex-col justify-between font-mono text-[10px] pr-2" style={{ color: "#6B6B73" }}>
          {tickValues.map((v) => (
        ...
        <div ref={barsRef} className="flex-1 relative">
          ...
          <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
            {tickValues.map((_, i) => (
              <div
                key={i}
                style={{
                  borderTop: i === tickValues.length - 1 ? "none" : "1px dashed rgba(255,255,255,0.05)",
                  height: 0,
                }}
              />
            ))}
          ...
          {avg > 0 && (
            <div
              ...
              style={{ bottom: `${(avg / niceMax) * 100}%`, height: 0 }}
            >
              <div className="flex-1" style={{ borderTop: "1px dashed rgba(244,201,93,0.45)" }} />
              <span className="ml-2 text-[10px] font-mono" style={{ color: "#F4C95D" }}>
                Avg {formatINR(avg)}
              </span>
            </div>
          )}
          ...
          <div className="absolute inset-0 flex items-end gap-[3px] px-0.5">
            {series.map((d, i) => {
              ...
              const h = inFuture ? 0 : Math.max(2, (d.expense / niceMax) * 100);
              return (
                <div
                  key={d.day}
                  data-bar
                  ...
                  style={{
                    ...
                    boxShadow: inFuture ? "none" : "0 0 12px rgba(255,139,61,0.25)",
                  }}
                ...
                >
                  <div
                    ...
                    style={{ background: "rgba(0,0,0,0.7)", color: "#F5F2EA", border: "1px solid rgba(255,255,255,0.1)" }}
                  >
      ...
      {/* X axis labels */}
      <div className="mt-3 flex justify-between text-[10px] font-mono pl-9" style={{ color: "#6B6B73" }}>
        {[1, 5, 9, 13, 17, 21, 25, 29].map((d) => (
          <span key={d} className="tabular">{d}</span>
        ))}
      </div>
    </motion.section>
  );
